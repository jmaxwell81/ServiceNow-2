//Script Include LoanerResUtil
//To use object with constructor notation var lu = new LoanerResUtil();

var LoanerResUtil = Class.create();

LoanerResUtil.prototype = {
  initialize: function() {  
    },
  getOverdueReservations : function() {

    var a = new Array();
    var gr = new GlideRecord('u_loaner_reservation');
 
    gr.addQuery('u_end_date', '<=', gs.nowDateTime());
    gr.addQuery('u_loaner_state', 2);
    gr.query();
  ...
  isAvailable : function(ci, start, end) {
    ...
    var ci_id = ci;
    var count = 0;
	var lr = new GlideAggregate('u_loaner_reservation');
    ...
    myQuery += 'u_start_date<=' + end + '^';
    myQuery += 'u_end_date>=' + start + '^';
    myQuery += 'NQ';
    myQuery += 'active=true^';
    myQuery += 'cmdb_ci.sys_id=' + ci + '^';
    myQuery += 'u_end_date<=' + gs.daysAgo(0);
  ...
  availableCis : function(class_name, pickup_location, manufacturer, start, end) {
	...
	//Populate the reference qualifier items return sys_ids in a comma separated list
	  gs.log("Available items array: " + availableItems);
