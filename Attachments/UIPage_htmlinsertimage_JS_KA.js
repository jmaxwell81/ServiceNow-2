//UI Page html_insert_image_dialog
initImageDialog();
     ...
     if (param["f_url"]) {
       ...
       if (!isDbImage) {
             if (!isAttachmentImage) {
                ...
                gel("f_url").focus();
                isURL = true;
                //EDIT KA set link type to attachment
                //setLinkType('url');
                setLinkType('attachment')
         }
       }
...
function scalePreviewImage() {
  ...
  if (previewImg) {
     ...
     //EDIT KA set max size for preview image
     var maxWidthPvw = 300;
     var maxHeightPvw = 200;
     if (width > maxWidthPvw || height > maxHeightPvw) { // need to scale back?
        var widthRatioPvw = (width - maxWidthPvw) / width;
        var heightRatioPvw = (height - maxHeightPvw) / height;
        var ratioPvw = 1 - Math.max(widthRatioPvw, heightRatioPvw);
        var scaledWidthPvw = parseInt(width * ratioPvw);
        var scaledHeightPvw = parseInt(height * ratioPvw);
        previewImg.width = scaledWidthPvw;
        previewImg.height = scaledHeightPvw;
        //height = scaledHeight;
     }
  //EDIT KA scale image size
      ...
      if (width > maxWidth || height > maxHeight) { // need to scale back?
        ...
        gel("f_width").value = scaledWidth;
        gel("f_height").value = scaledHeight;
      }
	//EDIT KA
	  //Else if image does not need to be resized then set size fields to blank to keep the original size
